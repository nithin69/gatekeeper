...
telangana = "Telangana"
state_choices = ((andhra_pradesh, 'Andhra Pradesh'), (telangana, 'Telangana'))
...
class EmailConfirmed(models.Model):
	...
	def email_user(self, subject, message, from_email=None, **kwargs):
            s = SMTP()
            s.connect('smtp.mymaths.in')
            s.login('contact@mymaths.in','ramesh123')
            print self.user.email
            s.sendmail('contact@mymaths.in', self.user.email, message)
